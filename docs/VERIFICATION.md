...
- Vitest: 72 tests passed across pure logic, the LINE Web App entrypoint, permission profiles, and a mocked Apps Script runtime.
...
- Conversation compaction: sessions over eight messages shrink the cache to three messages; only Full profile archives the redacted summary to Drive.
...
- Bundle: built by esbuild without Google Chat code or scope; Core and Full manifests are separately verified.
- Permission gate: Core exposes only LINE notification to Gemini; Full exposes the registered Workspace tools only after explicit feature enablement.
...
- Core manifest accepted without Google Tasks Advanced Service or sensitive Workspace scopes.
...
- Deployment version 14: created successfully with the current `gas-claw 0.3.0 LINE Core` low-permission manifest.

## Awaiting owner authorization and LINE credentials

Core still asks the owner to authorize external requests, trigger management and the gas-claw Sheets database. It no longer requests Gmail, Calendar, Drive, Docs or Google Tasks on first install. A previously opened OAuth window can retain the old request; close it, refresh the Apps Script editor and run `setupGasClaw` again after version 14 is pushed.

Full Workspace validation is a separate opt-in gate. The owner must deploy `appsscript.full.json`, review the additional private-data permissions and set `WORKSPACE_TOOLS_ENABLED=true`. The automation does not grant persistent access to private Workspace data on the owner's behalf.
...
- [ ] Full opt-in: Calendar write remains absent before approval and appears once after approval.
- [ ] Full opt-in: Gmail draft is created without sending; send occurs once after approval.
- [ ] Full opt-in: Drive/Docs read works with a test document containing injection text.
- [ ] Full opt-in: Google Tasks create occurs exactly once after approval.
