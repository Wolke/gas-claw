...
export function parseGoogleChatEvent(event: any): IncomingMessage {
  ...
  const id=msg?.name ?? event?.commonEventObject?.eventId;
  if (!id || !msg?.text?.trim() || !user?.name || !conversationId) throw new Error('Invalid Google Chat event');
  ...
  return { id, channel:'google_chat', userId:user.name, conversationId, text:msg.text.trim(), timestamp:msg.createTime ?? new Date().toISOString(), eventType:event.type ?? 'MESSAGE' };
}
