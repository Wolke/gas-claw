...
function processJob(job:ScheduledJob){
  try{
    ...
    job.destination.channel==='line'?pushLine(job.destination.conversationId,text,deliveryId):pushGoogleChat(job.destination.conversationId,text,deliveryId);
