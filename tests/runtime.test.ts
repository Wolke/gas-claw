...
beforeEach(()=>{
  sheets=Object.fromEntries(Object.entries(headers).map(([k,v])=>[k,new FakeSheet(v)]));properties={DATABASE_SPREADSHEET_ID:'db',LINE_OWNER_ID:'owner',GOOGLE_CHAT_OWNER_ID:'users/owner',LINE_CHANNEL_ACCESS_TOKEN:'test-token'};cache=new Map();uuid=0;lockHeld=false;
