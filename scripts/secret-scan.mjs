...
const findings=[];
for(const file of files){if(!existsSync(file))continue;const text=readFileSync(file,'utf8');for(const pattern of patterns){for(const match of text.matchAll(pattern)){findings.push(`${file}:${text.slice(0,match.index).split('\n').length}: ${match[0].slice(0,24)}…`)}}}
